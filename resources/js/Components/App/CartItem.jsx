...
const CartItem = ({item}) => {
  ...
  const handleQuantityChange = (ev) => {
    ...
    router.put(route('cart.update', item.product_id), {
      ...
      preserveScroll: true,
      onError: (errors) => {
        setError(Object.value(errors)[0]);
      }
    })
  }
  return (
    <>
      <div key={item.id} className="flex gap-6 p-3">
        <Link href={productRoute(item)}
              ...
              <img src={item.image} alt="" className="max-x-full max-h-full" />
        </Link>
        <div className="flex flex-col flex-1">
          ...
          <div className="flex justify-between items-center mt-4">
            <div className="flex gap-2 items-center">
              ...
              <div className={error ? 'tooltip tooltip-open tooltip-error' : ''} data-type={error}>
